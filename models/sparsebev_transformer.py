...
class SparseBEVTransformerDecoderLayer(BaseModule):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, bev_feats):
        ...
        query_bev_feat = self.sampling_bev(query_bbox, query_feat, bev_feats, img_metas)

        query_feat_cam = self.norm2(self.mixing(sampled_feat, query_feat))

        query_feat_bev = self.norm_bev(self.mixing_bev(query_bev_feat, query_feat))

        query_feat = self.fuse_mlp(torch.cat([query_feat_cam, query_feat_bev], dim=-1))  # [B, Q, C*2]
