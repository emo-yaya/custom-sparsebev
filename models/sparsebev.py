...
class SparseBEV(MVXTwoStageDetector):
    ...
    def simple_test(self, img_metas, img=None, rescale=False, img_inputs=None):
        ...
        if world_size == 1 and False:  # online
            return self.simple_test_online(img_metas, img, rescale, img_inputs)
        else:  # offline
            return self.simple_test_offline(img_metas, img, rescale, img_inputs)

    def simple_test_offline(self, img_metas, img=None, rescale=False, img_inputs=None):
        img_feats, bev_feat = self.extract_feat(img=img, img_metas=img_metas, img_inputs = img_inputs[0])

        bev_feat = self.fuse_history(bev_feat, img_metas, img_inputs[0][6])
    ...
    def simple_test_online(self, img_metas, img=None, rescale=False, img_inputs=None):
        ...
        for i in range(num_frames):
            ...
            if img_filenames[img_indices[0]] in self.memory:
                ...
                img_feats_curr, bev_feat = self.extract_feat(img[:, i], img_metas_curr, img_inputs[0])
                bev_feat = self.fuse_history(bev_feat, img_metas, img_inputs[0][6])
