...
class CustomNuScenesDataset(NuScenesDataset):    
    def __init__(self, occ_gt_root, *args, **kwargs):
        super().__init__(filter_empty_gt=True, *args, **kwargs)
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, bbox_result=None, **eval_kwargs):
        bbox_result = [out['pts_bbox'][0] for out in occ_results]
        metrics = super().evaluate(bbox_result, jsonfile_prefix='submission')
        ...
        for batch in tqdm(DataLoader(EgoPoseDataset(self.data_infos), num_workers=8)):
            token = batch[0][0]
            ...
            if data_type == 'occ3d' or data_type == 'occ3d_panoptic' or data_type == 'gts':
        ...
        if len(inst_preds) > 0:
            ...
            return main_rayiou(occ_preds, occ_gts, lidar_origins, occ_class_names=occ_class_names), metrics

    def format_results_occ(self, occ_results, submission_prefix, **kwargs):
