...
class PrepareImageInputs(object):
    ...
    def get_inputs(self, results, scale=None):
        ...
        for cam_name in cam_names:
            index = results['cam_names'].index(cam_name)
            filename = results['img_filename'][index]
            img = Image.open(filename)
            post_rot = torch.eye(2)
            post_tran = torch.zeros(2)

            intrin = torch.Tensor(results['cam_intrinsic'][index])

            sensor2keyego, sensor2sensor = \
                self.get_sensor2ego_transformation(results,
                                                   results,
                                                   cam_name,
                                                   self.ego_cam, index)
            ...
            sensor2ego, ego2global = \
                self.get_sensor_transforms(results, cam_name, index)
            ...
            if self.sequential:
                ...
                for adj_info in results['adjacent']:
                    filename_adj = adj_info[cam_name]['data_path']
            ...
            for adj_info in results['adjacent']:
                ...
                for cam_name in cam_names:
                    ...
                    adjsensor2keyego, sensor2sensor = \
                        self.get_sensor2ego_transformation(adj_info[cam_name],
                                                           results,
                                                           cam_name,
                                                           self.ego_cam, index)
                ...
                for cam_name in cam_names:
                    sensor2ego, ego2global = \
                        self.get_sensor_transforms(adj_info[cam_name], cam_name)
                    sensor2egos.append(sensor2ego)
                    ego2globals.append(ego2global)
